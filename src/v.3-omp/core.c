...
double recursive(const double* rxyz, double* fxyz, double* epot, double* pres_vir,
    double rcut2, const double L, const int ii,const int fi,const int ij,const int fj)
{    
    // Tamaño de Bloque
    if(fi-ii<=BLOCK){
        double gf = 0.0;
        ...
        for(int i = ii;i<(f);i++){
            int d = ij>i?ij:i+1;
            gf += dist(rxyz,fxyz,epot,pres_vir,L,i,rcut2,d,fj+1);
        }
    ...
    //Recursión 
    double gf1,gf2,gf3,gf4=0.0;
    double ept1=0.0,ept2=0.0,ept3=0.0,ept4=0.0;
    double prest1=0.0,prest2=0.0,prest3=0.0,prest4=0.0;
    int mitadi = ii + ceil((fi-ii)/2);
    int mitadj = ij + ceil((fj-ij)/2);

    #pragma omp task shared(gf1,fxyz,ept1,prest1)
    gf1=recursive(rxyz,fxyz,&ept1,&prest1,rcut2,L,ii,mitadi,ij,mitadj);
    #pragma omp task shared(gf2,fxyz,ept2,prest2)
    gf2=recursive(rxyz,fxyz,&ept2,&prest2,rcut2,L,mitadi,fi,mitadj,fj);
    
    #pragma omp taskwait

    #pragma omp task shared(gf3,fxyz,ept3,prest3)
    gf3 = recursive(rxyz,fxyz,&ept3,&prest3,rcut2,L,ii,mitadi,mitadj,fj);
    if(mitadj>mitadi){
        #pragma omp task shared(gf4,fxyz,ept4,prest4)
        gf4 = recursive(rxyz,fxyz,&ept4,&prest4,rcut2,L,mitadi,fi,ij,mitadj);
    }
    
    #pragma omp taskwait

    #pragma omp atomic
    *epot += ept1+ept2+ept3+ept4;
    #pragma omp atomic
    *pres_vir += prest1+prest2+prest3+prest4;
    

    return gf1 + gf2 + gf3 + gf4;
}
            ...
            const double* temp, const double rho, const double V, const double L)
{
    ...
    *epot = 0.0;
    ...
    #pragma omp parallel shared(fxyz,epot,pres_vir)
    {
    #pragma omp single
    flop += recursive(rxyz,fxyz,epot,&pres_vir,rcut2,L,0,N,0,N);
    }
    /*
    for(int bj=0;bj<N;bj+=BLOCK){
        #pragma omp parallel for ordered shared(rxyz,fxyz,epot,pres_vir) reduction(+:flop) 
        for(int bi=0;bi<N;bi+=BLOCK){
            int m = N<(bi+BLOCK)?N:bi+BLOCK;
            for (int i = bi; i < (m - 1); i++) { //(N-1) iteraciones
                // Loop NO autovectorizado:
                int d = bi+bj>i?bi+bj:i;
                #pragma omp ordered
                flop += dist(rxyz,fxyz,epot,&pres_vir,L,i,rcut2,d,bi+bj+BLOCK);
            }
        }
        // barrera implicita parallel for
    }
    */
    //printf("PRESION = %f\n",pres_vir);
