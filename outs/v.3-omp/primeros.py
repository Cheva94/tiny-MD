...
for h in [1, 7, 14, 21, 28]:

    fig, ax = plt.subplots(figsize=(12.5,10))

    ax.set_title(f'# Hilos = {h}')

    h_h = pd.read_csv(f'/home/cheva/tiny_md/outs/v.3-omp/primeros_hilos-{h}.csv', comment = '#')
    ...
    for part in h_h.iloc[:, 0].drop_duplicates().to_numpy():
        ...
        ax.errorbar(x, y, yerr=yerr, capsize=8, elinewidth=5, capthick = 2,
                    linestyle='--', fmt='o', markersize=10, label=f'# Part={part}')
...
#    plt.savefig(f'/home/cheva/tiny_md/outs/v.3-omp/primeros_hilos-{h}.png')
